...
export default function SeedData() {
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      // 6. Seed Plans (Admin part)
      const { data: plans, error: plansError } = await supabase
        .from("plans")
        .insert([
          { name: "Basic", price: 999, currency: "INR", trial_days: 14, status: "active", description: "Basic plan for small shops" },
          { name: "Pro", price: 2499, currency: "INR", trial_days: 30, status: "active", description: "Pro plan for medium businesses" },
          { name: "Enterprise", price: 4999, currency: "INR", trial_days: 0, status: "active", description: "Enterprise plan for large retailers" },
        ])
        .select();
  ...
  return (
    <div className="p-6 border rounded-lg">
      <h3 className="text-xl font-semibold mb-4">Seed Dummy Data</h3>
      <p className="text-muted-foreground mb-4">
        Click the button below to add dummy data to your database (categories, products, customers, suppliers, etc.)
      </p>
